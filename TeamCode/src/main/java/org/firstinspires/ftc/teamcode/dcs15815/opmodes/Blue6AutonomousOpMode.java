...
@Autonomous(name = "Blue Far 9a", group = "9a", preselectTeleOp = "Driver Operated")
public class Blue6AutonomousOpMode extends DecodeAutonomousOpMode {
		...
		.setDebugging(true)
	   	.startWithState(
			...
			DecodeShootState
			 ...
			 DecodeShootState
				    .make()
				    .setBoost(DecodeConfiguration.BLUE6_SHOOTER_BOOST_SECOND_SET)
				    .setLabel("SHOOT SECOND THREE")
	   ).andThen(
			 DriveToPositionState
				    .where("BLUE6_LINE2_START")
				    .setLabel("DRIVE TO LINE 2")
	   ).andThen(
			 DriveToPositionState
				    .where("BLUE6_LINE2_END", DecodeConfiguration.INTAKE_TIME_WAIT_AUTONOMOUS)
				    .setPowerOverride(DecodeConfiguration.BLUE6_DRIVETRAIN_POWER_INTAKE)
				    .setBeforeStart(() -> {
					    bot.intake.turnOn();
				    })
				    .setBeforeStop(() -> {
			 ...
			 DecodeShootState
				    .make()
				    .setBoost(DecodeConfiguration.BLUE6_SHOOTER_BOOST_THIRD_SET)
