...
@Autonomous(name = "Blue 4", group = "4", preselectTeleOp = "Driver Operated")
...
//	static final Pose2D START_OF_LINE_1 = new Pose2D(DistanceUnit.INCH, 24, -12, AngleUnit.DEGREES, -135);
	...
	public void performAutonomous() {
		...
		bot.navigation.setPosition(DecodeConfiguration.BLUE1_START);
		...
		while (opModeIsActive()) {
			...
			switch (currentState) {
				case STARTUP_SHOOTER:
					...
					bot.shooter.changeShooterPower(0.985);
...
//					target = FIRST_SHOOTING_POSITION;
					if (bot.navigation.driveToFromCurrent(DecodeConfiguration.BLUE1_SHOOT_PRELOAD.asPose2D())) {
